...
function buildStatusEmbed(mc: any, serverIp: string) {
  ...
  return {
    title: online ? "🟢 CarnageMC — Online" : "🔴 CarnageMC — Offline",
    ...
    footer: { text: "CarnageMC · Live status (auto-updated)" },
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "announce") {
      ...
      const message = body.message || "📢 **Test announcement** from the CarnageMC admin panel.";
    ...
    if (action === "welcome") {
      ...
      const template = body.message || cfg.welcomeMessage || "Welcome to CarnageMC, {user}!";
    ...
    if (action === "roles") {
      ...
      const embed = applyOverride({
        title: "CarnageMC — Server Roles",
        description: lines,
        color: 0x5865f2,
        footer: { text: "CarnageMC · Roles overview" },
        timestamp: new Date().toISOString(),
      }, "roles");
    ...
    if (action === "info" || action === "rules") {
      ...
      if (action === "info") {
        embed = applyOverride({
          title: "🍸 CarnageMC — Server Info",
          description:
            "Welcome to **CarnageMC** — your home for premium survival, minigames, and community events.\n\nConnect using the details below and join the adventure!",
          color: 0xff7a1a,
          ...
          footer: { text: "CarnageMC · See you in-game!" },
          timestamp: new Date().toISOString(),
        }, "info");
