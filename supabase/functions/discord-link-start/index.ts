...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { error: insErr } = await admin.from("discord_link_states").insert({
    ...
    const stateWithReturn = returnTo ? `${state}.${btoa(returnTo)}` : state;
    ...
    url.searchParams.set("state", stateWithReturn);
